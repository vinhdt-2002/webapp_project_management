import { Column, Comment, Task, Work } from "../models/index.js";
...
export const createTask = async (req, res) => {
  try {
    ...
    if (!findColumn) return res.status(400).json({ err: "Invalid column ID" });
...
export const getTaskByID = async (req, res) => {
  try {
    ...
    return res.status(200).json({ msg: "Data retrieved successfully.", task });
  } catch (error) {
    res.status(500).json({ err: error.message });
  }
};
...
export const updateTask = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ msg: "Data updated successfully.", task: updateTask });
  } catch (error) {
    res.status(500).json({ err: error.message });
  }
};
