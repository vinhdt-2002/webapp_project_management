...
export default function isAuth(req, res, next) {
  ...
  if (!token) {
    return res.status(400).json({ err: "Authen error." });
  }
  jwt.verify(token, process.env.AC_TOKEN, async (err, decoded) => {
    if (err) return res.status(400).json({ err: "Authen error." });
    const { id } = decoded;
    const findUsername = await User.findById(id).select("-password");
    if (!findUsername) return res.status(400).json({ err: "Authen error." });
