  ...
  function handleAddUser(id, username) {
    dispatch(
      updateTask({
        ...
        content: `Added ${username} to task ${task.data.title}`,
      })
    );
    setValue("");
    setResultMembers([]);
  ...
  function handleRemoveUser(id, username) {
    ...
    dispatch(
      updateTask({
        ...
        idProject: project.data._id,
        idTask: task.data._id,
        content: `Removed ${username} from task ${task.data.title}`,
      })
    );
    setValue("");
    setResultMembers([]);
      ...
      <div className="managerMembers">
            ...
            <ul className="managerMembers__input-result">
              {resultMembers.map((el, i) => (
                <li key={i}>
                  ...
                  <p>{el.username}</p>
                  {task.data.members.findIndex((el1) => el1._id === el._id) ===
                    -1 && (
                    <div
        ...
        <div className="managerMembers__members">
          <p className="managerMembers__members-title">
            Total: {task.data?.members.length} members
          </p>
          <ul>
                  ...
                  <div className="dropdown__content">
                    {!project.data?.admins.includes(el._id) && (
                      <>
                        <div
                          className="item"
                          onClick={() => handleRemoveUser(el._id, el.username)}
                        >
