  ...
  function handleAddUser(id, username) {
    dispatch(
      updateProjectv2({
        ...
        content: `Added ${username} to the project`,
      })
    );
    setValue("");
    setResultMembers([]);
  ...
  function handleRemoveUser() {
    ...
    dispatch(
      updateProjectv2({
        ...
        token: auth.token,
        idProject: project.data._id,
        content: `Removed ${username} from the project`,
      })
    );
    setModelDelete(false);
  }
  ...
  function handleAddAdmins(id) {
    ...
    dispatch(
      updateProjectv2({
        ...
        token: auth.token,
        idProject: project.data._id,
        content: `Made ${username} an admin`,
      })
    );
  }
    ...
    <Model close={close}>
      {modelDelete && (
        <CustomDeleteModel
          ...
          msg={"Are you sure you want to remove this member from the project?"}
        />
      )}
      <div className="managerMembers">
              ...
              {resultMembers.map((el, i) => (
                <li key={i}>
                  ...
                  {project.data.members.findIndex(
                    ...
                    <div
                      onClick={() => handleAddUser(el._id, el.username)}
                      className="add-user"
                    >
        ...
        <div className="managerMembers__members">
          <p className="managerMembers__members-title">
            Total: {project.data?.members.length} members
          </p>
          <ul>
            {project.data?.members.map((el, i) => (
              <li key={i} className="managerMembers__member">
                ...
                {project.data?.admins.includes(el._id) && (
                  <div className="managerMembers__member-admin">Admin</div>
                )}
